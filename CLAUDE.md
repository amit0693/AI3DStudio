...
**Commerce API routes** (`app/api/*/route.ts`) are plain Fetch API `Request`/`Response` handlers with no framework middleware. Shared request/response plumbing lives in `app/api/products/_shared.ts` — `ApiError`, `json`/`handleApiError`, `readJsonObject`, `cleanText`/`cleanEmail`/`integerInRange`, `parseJsonColumn`, `sha256Hex`/`randomToken` — reuse these instead of reimplementing validation or error shaping in a new route.
